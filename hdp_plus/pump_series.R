...
generate_bins_df <- function(pristine_df, chunk_size, event_datetime) {
  ...
  before_event <- rev(seq(event_datetime,
                          datetime_range[1],
...
cache_pump_time_series <- function(pump_file, chunk_size, file_name) {
  ...
  orders_history_df <- read.csv(paste0("lamorgia/binance_orders_history/",
                                       pump_file))
  ...
  btc_usd_price_history <- get_btc_usd_price_history(
    min(buy_orders_history_df$datetime),
    max(buy_orders_history_df$datetime))
...
get_pump_time_series <- function(pump_file_name, chunk_size) {
  ...
  base_path <- paste0("cached_data/pre_processed_series/",
                      "chunk_size-",
                      chunk_size,
                      "/")
  target_file_name <- paste0(pump_file_name,
                             "_",
                             chunk_size,
                             "s.csv")
  
  cached_files <- list.files(base_path, pattern = "*.csv")
  if (!(target_file_name %in% cached_files)) {
    ...
    cache_pump_time_series(paste0(pump_file_name, ".csv"),
                           chunk_size,
                           paste0(base_path, target_file_name))
  }
  
  return(read.csv(paste0(base_path, target_file_name)))
}
